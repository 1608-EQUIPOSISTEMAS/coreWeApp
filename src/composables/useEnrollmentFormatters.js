export function useEnrollmentFormatters () {
  ...
  const cuotaRowClass = c => {
    if (isCuotaAnulada(c)) return 'cuota-annulled'
  ...
  const cuotaStatusPill = (c, planStatus) => {
    if (isCuotaAnulada(c)) return 'pill-muted'
  ...
  const cuotaStatusLabel = (c, planStatus) => {
    if (isCuotaAnulada(c)) return 'Anulada'
  ...
  const auditLabel = action => {
    const map = { created: 'Inscripcion creada', approved: 'Pago aprobado', edited: 'Datos editados', odoo_enrolled: 'Inscrito en Odoo', odoo_unenrolled: 'Desinscrito de Odoo', email_sent: 'Correo enviado', email_failed: 'Error al enviar correo', payment_registered: 'Pago registrado', edition_reprogrammed: 'Edicion reprogramada', course_changed: 'Cambio de curso', created_from_cc: 'Creado por cambio de curso', created_from_rp: 'Creado por reprogramacion', children_created: 'Modulos hijos creados', children_skipped_no_edition: 'Modulo saltado: sin edicion', parent_marked_e0: 'Padre marcado E0', modality_changed: 'Cambio de modalidad', profile_changed: 'Cambio de perfil', student_edited: 'Datos del alumno editados', seller_agent_changed: 'Asesor actualizado', retired: 'Alumno retirado', observed: 'Inscripcion observada', resubmitted: 'Reenviado a FICO', created_from_token: 'Creado desde token de pago', validation_requested: 'Convalidacion solicitada', validation_applied: 'Convalidacion aplicada', installments_rescheduled: 'Cuotas reprogramadas', odoo_fee_paid: 'Cuota marcada pagada en Odoo', odoo_fees_activated: 'Cuotas Odoo activadas', token_inscription_edited: 'Inscripcion editada por asesor', token_grouped: 'Token agrupado', token_ungrouped: 'Token desagrupado', token_link_added: 'Link de pago colocado', token_link_edited: 'Link de pago editado', installment_amount_edited: 'Monto de cuota editado', installment_added: 'Cuota agregada', additional_payment: 'Pago de certificado registrado', additional_payment_edited: 'Pago de certificado editado', collection_campaign: 'Campaña de cobranza aplicada' }
