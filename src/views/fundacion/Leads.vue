...
<tr v-if="!isCompact" class="thead-filter">
  ...
  <th class="tf">
    <MultiSelect v-if="!isFundacion" v-model="filters.owner_user_ids" :items="filtroOwners" label-key="description" value-key="id" placeholder="Todos..." class="hf-multiselect" @update:model-value="triggerInlineFilter" />
...
<tr v-if="isCompact" class="thead-filter">
  ...
  <th v-show="colGroups.asesor" class="tf">
    <MultiSelect v-if="!isFundacion" v-model="filters.owner_user_ids" :items="filtroOwners" label-key="description" value-key="id" placeholder="Asesor..." class="hf-multiselect" @update:model-value="triggerInlineFilter" />
  </th>
  <th v-show="colGroups.asesor" class="tf">
...
<tbody v-else>
  ...
  <BaseModal v-model="showControlModal" :title="isFundacion ? 'Mis Permisos de Visualización' : 'Panel de Control: Restricciones de Asesores'" size="xl">
    <div v-if="!isFundacion" class="px-4 py-3">
        ...
        <table class="exec-table">
          <thead>
            <tr class="thead-group">
              <th rowspan="2" class="th-cat sticky-col" style="min-width: 200px;">Asesor Fundación</th>
...
const FUNDACION_ROLES = ['FUNDACION', 'LIDER_FUNDACION']
const fundacionOwnerIds = ref([])
const isFundacion = storedUser?.roles?.includes('FUNDACION') &&
...
// el SP solo devuelve leads de la linea de negocio Fundacion. Se resuelve por
...
const COL_GROUPS_KEY = 'crm_leads_fundacion_col_groups_v2'
...
// === PERSISTENCIA ===
const { saveState } = useTablePersistence('crm_leads_fundacion_filter_state_v1', filters, pagin)
...
async function openControlModal() {
  ...
  try {
    ...
    const savedRestrictions = await comercialService.restrictionsList({ user_id: currentUserId, is_comercial: isFundacion });
...
function rebuildChips() {
  ...
  if (!isFundacion) makeChip('owner_user_ids', 'Asesor', filters.owner_user_ids)
...
function buildLeadPayload() {
  ...
  // Sin seleccion explicita de asesor, el universo es el de Fundacion (no "todos").
  ...
  const ownerIds = pickedOwners.length
    ? pickedOwners
    : (fundacionOwnerIds.value.length ? fundacionOwnerIds.value : [-1])
  return {
...
function clearFilters(reload = true) {
  ...
  if (isFundacion && currentUserId) filters.owner_user_ids = [currentUserId]
  if (reload === true || typeof reload !== 'boolean') {
    activeQuickView.value = null
    pagin.value.page = 1
    localStorage.removeItem('crm_leads_fundacion_filter_state_v1')
    rebuildChips()
    fetchLeads()
  }
}
...
// Este modulo solo muestra las consultas de Fundacion: sus asesores son los
// usuarios con rol FUNDACION o LIDER_FUNDACION. El SP de listado no sabe de
// roles, asi que traducimos rol -> user_ids aqui y lo mandamos como
// owner_user_ids (ver buildLeadPayload).
async function loadOwners() {
  try {
    const lists = await Promise.all(
      FUNDACION_ROLES.map(r => authService.userListByRole(r).catch(() => [])),
    )
    const byId = new Map()
    ...
    fundacionOwnerIds.value = [...byId.keys()]
...
function goNew() { router.push({ name: 'FundacionLeadsNew' }) }

function editLead(lead, event) {
  const resolved = router.resolve({ name: 'FundacionLeadsEdit', params: { id: lead.id } })
  if (event?.ctrlKey) {
...
function viewLead(lead, event) {
  const resolved = router.resolve({ name: 'FundacionLeadsNew', query: { clone_from: lead.id } })
  if (event?.ctrlKey) {
    window.open(resolved.href, '_blank')
  } else {
    router.push(resolved)
...
onMounted(async () => {
  const weBL = catalog.options('we_business_line') || []
  businessLineId.value = weBL.find(e => e.alias === 'we_business_line_fundacion')?.id || null
  ...
  if (isFundacion && currentUserId) {
    filters.owner_user_ids = [currentUserId]
    checkMyRestrictions()
  }
  await loadOwners()   // fetchLeads necesita fundacionOwnerIds ya resuelto
  loadOriginPhones()
