...
<tbody v-else>
  ...
  <BaseModal v-model="showControlModal" :title="isB2B ? 'Mis Permisos de Visualización' : 'Panel de Control: Restricciones de Asesores'" size="xl">
    <div v-if="!isB2B" class="px-4 py-3">
  ...
  <div class="col-md-6">
    ...
    <template #footer>
      <div class="d-flex justify-content-end w-100 gap-2">
        <button class="btn-exec btn-exec-outline px-4" @click="showControlModal = false">{{ isB2B ? 'Entendido, cerrar' : 'Cancelar' }}</button>
        <button v-if="!isB2B" class="btn-exec btn-exec-warning px-4" @click="saveControlRestrictions" :disabled="isSavingRestrictions">
          <i class="fa-solid fa-save me-1"></i>
...
async function openControlModal() {
  ...
  try {
    ...
    const savedRestrictions = await comercialService.restrictionsList({ user_id: currentUserId, is_comercial: isB2B });
...
function rebuildChips() {
  ...
  if (!isB2B) makeChip('owner_user_ids', 'Asesor', filters.owner_user_ids)
...
function goNew() { router.push({ name: 'B2BLeadsNew' }) }

function editLead(lead, event) {
  const resolved = router.resolve({ name: 'B2BLeadsEdit', params: { id: lead.id } })
  if (event?.ctrlKey) {
...
function viewLead(lead, event) {
  const resolved = router.resolve({ name: 'B2BLeadsNew', query: { clone_from: lead.id } })
  if (event?.ctrlKey) {
    window.open(resolved.href, '_blank')
  } else {
    router.push(resolved)
...
onMounted(async () => {
  ...
  businessLineId.value = weBL.find(e => e.alias === 'we_business_line_b2b')?.id || null
